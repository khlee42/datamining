...
# Package and Data
For this assignment, we need three packages to load: `tidyverse`, `caret`, and `caTools`. You also need to download this data file "customer_churn.csv" ([click here](http://khlee42.github.io/datamining/homework/hw2/customer_churn.csv)) and save it in the same folder where this RMD file is. The below code chunk is to load these packages and the data.
...
- Split `churn` data and save 80% into `train` and 20% into `test` datasets (Hint: `createDataPartition()`).
```{r}
...
- Using `glm()`, build a logistic regression model to predict customer churn (`Churn`) with all possible variables. Use `train` data set to fit the model. Save the model as `mod_glm`
```{r}
# write your code below this comment line
...
- For `test` data, make prediction using the logistic regression trained in Question 2 (`mod_glm`) and save the prediction as `p` (Hint: `predict()`).
...
- For the full dataset (`churn`, not `train`), build a decision tree to predict customer churn (`Churn`) by using all possible variables. Use `train()` for building the model. Save the model as `mod_tree`.
```{r}
set.seed(123) # Do not change this line
# write your code below this comment line
