...
- Q1-1. Copy your data frame to `flights1`
```{r}
flights1 <- flights
```

- Q1-1. Add a new variable, total_delay, which is the sum of `dep_delay` and `arr_delay`, to your data frame
```{r}
flights1 <- flights1 %>%
  mutate(total_delay = dep_delay + arr_delay)
```

- Q1-2. Remove the columns `year` from your data frame [Hint: Reassign a data frame excluding the variables. You may want to use negative index]
```{r}
flights1 <- flights1 %>%
  select(-year)
```

- Q1-3. Remove the carriers with less than 1000 flights in 2014.
...
- Q1-4. Sort your data frame in descending order of total_day, and save it as a new data frame `flight2`
```{r}
flights2 <- flights1 %>%
